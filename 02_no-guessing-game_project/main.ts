...
async function number_guess_game() {
   ...
   in one round you earn 10 points`))
    ...
    let rounds: string[] = ["Round-1","Round-2","Round-3"];
    let earned_points = 0;

    for (let a = 0; a < rounds.length; a++) {
        console.log(chalk.inverse.magentaBright(`\n <<<<<<<<<<  ${(rounds[a])} >>>>>>>>>> \n`));

        let random_number: number = Math.round(Math.random()* 5);
        let user_input: number = parseInt(prompt(chalk.blueBright.bold("Enter any number between 0 and 5: ")));
